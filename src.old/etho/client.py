...
def client(
    ...
    *,
    ...
    if "THUA" in prot["use_services"] and not preview:
        ...
        thua.setup(prot["THUA"]["port"], prot["THUA"]["interval"], this["maxduration"])
        thua.init_local_logger("{0}/{1}/{1}_thu.log".format(this["datafolder"], save_prefix))
        services["THUA"] = thua
    ...
    for gcm_cnt, gcm_key in enumerate(gcm_keys):
        ...
        gcm.setup(
            f"{this['datafolder']}/{save_prefix}/{save_prefix}{save_suffix}",
            maxduration,
            cam_params,
        )

        if not preview:
            gcm.init_local_logger(f"{this['datafolder']}/{save_prefix}/{save_prefix}{save_suffix}_gcm.log")
    ...
    for daq_cnt, daq_key in enumerate(daq_keys):
        ...
        attenuation = None
        calibration = None
        if prot[daq_key]["calibration_file"] is not None:
            calibration = readconfig(f"{this['calibrationfolder']}/{prot[daq_key]['calibration_file']}")
        ...
        daq.init_local_logger(f"{this['datafolder']}/{save_prefix}/{save_prefix}{save_suffix}_daq.log")
    ...
    if "NIC" in prot["use_services"]:
        ...
        nic.init_local_logger(f"{this['datafolder']}/{save_prefix}/{save_prefix}{save_suffix}_daq.log")

    # display config info
    for key, s in services.items():
        rich_information(s.information(), prefix=key)

    logging.info("Copying playlist and protocol files to data folder ")
    shutil.copy(protocolfile, f"{this['datafolder']}/{save_prefix}/")
    if playlistfile is not None:
        shutil.copy(playlistfile, f"{this['datafolder']}/{save_prefix}/")
    ...
    time_last_cam_started = time.time() - 10  # in case no cam was initialized
    ...
    if daq_keys and "GCM" in prot["NODE"]["use_services"]:
        step = 0.1
        total = int(max(0, time.time() - time_last_cam_started + 5) / step)
        for _ in rich.progress.track(range(total), description='DAQ gives GCM a head start'):
            time.sleep(step)
        ...
        while RUN and not progress.finished:
            ...
            if stop_event is not None and stop_event.is_set():
                ...
                for task_name, task_id in tasks.items():
                    progress.stop_task(task_id)
                for service_name, service in services.items():
                    if service.status() not in ['finishing',  'finished']:
                        try:
                            logging.info(f"   {service_name}")
                            service.finish()
                        except:
                            logging.warning("     Failed but probably fine.")
                RUN = False

    for _ in rich.progress.track(range(5), description='Waiting for jobs to finalize'):
        time.sleep(1)
