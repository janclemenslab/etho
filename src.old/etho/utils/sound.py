    ...
    for row_number, (row_name, listitem) in enumerate(playlist.iterrows()):
        ...
        for stimIdx, stimName in enumerate(listitem.stimFileName):
            ...
            if len(x):
                if attenuation:
                    attenuate = None
                    gain = attenuate(float(listitem.intensity[stimIdx]), listitem.freq[stimIdx])
                else:
                    gain = float(listitem.intensity[stimIdx])
                x = x * gain
            ...
            xx[stimIdx] = x
        non_mirror_led_chan = [
            x for x in list(range(len(xx))) if x not in mirror_led_channel
        ][0]
