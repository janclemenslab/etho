...
class BaseCallback:
    ...
    def start(self):
        self.RUN = True
        logging.warning(f"{self.FRIENDLY_NAME} starting")
        self._run()

    def stop(self):
        logging.warning(f"{self.FRIENDLY_NAME} setting STOP")
        self.RUN = False
    ...
    def _run(self):
        ...
        while self.RUN:
            ...
            if data is not None:
                ...
                logging.warning(f"{self.FRIENDLY_NAME} stopping")
                self.stop()
                logging.warning(f"{self.FRIENDLY_NAME}   DONE")
        logging.warning(f"{self.FRIENDLY_NAME} cleanup")
        self._cleanup()
        logging.warning(f"{self.FRIENDLY_NAME}   DONE")
