...
from ethoservice.utils.ConcurrentTask import *
...
class PTG(BaseZeroService):
    ...
    def setup(self, savefilename, duration):
        ...
        if self.savefilename is None:  # display only - set up DISPLAY
            self.pDisplay = ConcurrentTask(task=disp, comms="pipe", taskinitargs=[self.frame_width, self.frame_height])
            ...
            for k, v in camera_info.items():
                dset.attrs[k] = v
            h5f.close()
            self.pWrite = ConcurrentTask(task=save, comms="queue",
                                           taskinitargs=[self.savefilename, self.frame_rate, self.frame_width, self.frame_height])
    ...
    def _worker(self, stop_event):
        ...
        while RUN: #and not stop_event.wait(0.0001):
            ...
            if self.savefilename is not None:
                ...
                self.pWrite.send(BGR)
            else:
                if frameNumber % 50 == 0:
                    self.pDisplay.send(BGR)
