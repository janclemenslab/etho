...
from ..utils.sound import parse_table, load_sounds, build_playlist, select_channels_from_playlist, parse_pulse_parameters

from ..services.SndZeroService import SND
from ..services.CamZeroService import CAM
from ..services.ThuZeroService import THU
from ..services.Opt2ZeroService import OPT2
from ..services.RelayZeroService import REL
...
def clientcaller(host_name, ip_address, playlistfile, protocolfile, filename=None):
    ...
    prefix = "%(asctime)s.%(msecs)03d {0}@{1}:".format("CC", hostname)
    ...
    if "SND" in prot["NODE"]["use_services"]:
        ...
        unique_rows = playlist.to_string(header=False, index=False, index_names=False).split("\n")
        ...
        sounds = load_sounds(
            sound_playlist.iloc[unique_row_idx],
            fs,
        ...
        playlist_items, totallen = build_playlist(
            sounds, maxduration, fs, shuffle=shuffle_playback, sound_order=unique_row_inverse
        )
