...
class SLP(BaseZeroService):
    ...
    SERVICE_NAME = "SLP"  # short, uppercase, 3-letter ID of the service (must equal class name)
    ...
    def setup(self, duration):
        ...
        self.test_image = np.random.randint(0, 256, size=(100, 100, 3))
...
def cli(serializer: str = "default", port: Optional[str] = None):
    if port is None:
        port = SLP.SERVICE_PORT
    s = SLP(serializer=serializer)
